...
import { Shield, Lock, Mail, ArrowRight, Loader2, AlertTriangle, KeyRound } from 'lucide-react';
import { adminLogin, verifyAdminLoginOtpAction } from './actions';
...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (requireOtp) {
      ...
      const result = await verifyAdminLoginOtpAction(formData);
      if (result?.error) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-900 dark:bg-black p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-gray-800/80 backdrop-blur-xl border border-gray-700 p-8 rounded-2xl shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-400 text-sm">{requireOtp ? 'Xác thực bảo mật 2 lớp' : t('adminPortalDesc')}</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-1.5 text-center mb-4">
              <p className="text-sm text-gray-300">
                Mã OTP (6 chữ số) đã được gửi đến <span className="font-semibold text-white">{otpEmail}</span>. Mã sẽ hết hạn sau 5 phút.
              </p>
            </div>
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-300">Nhập mã OTP</label>
              <div className="relative">
                ...
                <input 
                  ...
                  className="pl-10 pr-4 py-2.5 w-full bg-gray-900/50 border border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 text-white placeholder-gray-600 transition-all text-center font-mono text-lg tracking-[0.5em]"
                />
              </div>
            </div>
            ...
            >
              {isLoading ? (
                ...
                <><ArrowRight className="w-4 h-4" /> Xác thực OTP</>
              )}
            </button>
            <div className="mt-4 text-center">
              <button type="button" onClick={() => { setRequireOtp(false); setErrorMsg(""); }} className="text-xs text-gray-500 hover:text-gray-300 transition-colors">
                Quay lại đăng nhập
              </button>
            </div>
          </form>
