...
export default function AdminFeedbackClient({ initialData }: { initialData: Record<string, unknown>[] }) {
  const t = useTranslations("Admin.Feedback");
  const [activeStatus, setActiveStatus] = useState("all");
  const mockFeedback = initialData || [];
          ...
          {mockFeedback.length === 0 ? (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800/50">
                {mockFeedback.map((item: Record<string, unknown>) => (
                  <tr key={item.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <Image src={item.avatar as string} alt="Avatar" width={32} height={32} className="w-8 h-8 rounded-full bg-gray-100 border border-gray-200 dark:border-gray-700" />
