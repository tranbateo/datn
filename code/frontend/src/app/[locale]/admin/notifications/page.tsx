...
const notifications = [
  {
    ...
    content: "Hệ thống AI Tutor sẽ tạm dừng hoạt động từ 23:00 - 01:00 để nâng cấp model.",
    type: "warning",
    target: "Toàn bộ học sinh",
    sentAt: "Today, 10:00 AM",
    readCount: "1,245 / 5,000"
  },
  {
    id: 2,
    title: "Cập nhật khóa học Toán 12",
    content: "Chương 4: Số phức đã được thêm vào hệ thống với 50 bài tập mới.",
    type: "info",
    target: "Khối 12",
    sentAt: "Yesterday, 14:30 PM",
    readCount: "890 / 1,200"
  },
  {
    id: 3,
    title: "Chúc mừng năm mới!",
    content: "EduAI kính chúc các bạn một năm mới tràn đầy niềm vui và đạt kết quả cao trong học tập.",
    type: "success",
    target: "Toàn bộ học sinh",
    sentAt: "Jan 01, 00:00 AM",
    readCount: "4,500 / 5,000"
  }
];
        ...
        <div className="w-full lg:w-2/3 space-y-6">
          ...
          <div className="bg-white dark:bg-card-bg rounded-2xl border border-gray-200 dark:border-card-border shadow-sm flex flex-col min-h-[500px]">
                  ...
                  {notifications.map((notif) => (
                    <tr key={notif.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                      <td className="px-6 py-4">
                        {notif.type === 'info' && <div className="w-8 h-8 rounded-full bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center"><Info className="w-4 h-4 text-blue-500" /></div>}
                        {notif.type === 'warning' && <div className="w-8 h-8 rounded-full bg-amber-50 dark:bg-amber-900/30 flex items-center justify-center"><AlertCircle className="w-4 h-4 text-amber-500" /></div>}
                        {notif.type === 'success' && <div className="w-8 h-8 rounded-full bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center"><CheckCircle2 className="w-4 h-4 text-emerald-500" /></div>}
                      </td>
                      <td className="px-6 py-4 max-w-xs truncate">
                        <p className="font-semibold text-gray-900 dark:text-white truncate">{notif.title}</p>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate">{notif.content}</p>
                      </td>
                      <td className="px-6 py-4 text-gray-600 dark:text-gray-400">
                        <div className="flex items-center gap-1.5 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded-md w-fit">
                          <Users className="w-3.5 h-3.5" /> <span className="text-xs font-medium">{notif.target}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                        <Clock className="w-3.5 h-3.5" /> {notif.sentAt}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="p-4 mt-auto flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 font-medium">
              <div>Showing 1-3 of 45 notifications</div>
