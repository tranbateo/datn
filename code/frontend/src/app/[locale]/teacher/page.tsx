...
import { Users, Library, FileQuestion, TrendingUp, BookOpen, AlertCircle } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-card-bg rounded-2xl p-6 border border-gray-100 dark:border-card-border shadow-sm">
          ...
          <h3 className="text-3xl font-bold text-gray-900 dark:text-white">8.5</h3>
        </div>
      </div>
          ...
          <div className="space-y-4">
            {[1, 2, 3].map((_, i) => (
              <div key={i} className="flex items-start gap-4 p-4 rounded-xl bg-red-50/50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-gray-900 dark:text-white">Student {i + 1}</h4>
                  <p className="text-xs text-red-600 dark:text-red-400 mt-1">Missed 3 assignments and score dropped by 20%.</p>
                </div>
              </div>
            ))}
          </div>
